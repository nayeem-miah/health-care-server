...
const login = async (payload: { email: string, password: string }) => {
    ...
    const accessToken = jwtHelpers.generateToken({ email: user.email, role: user.role }, config.jwt.access_token_secret as Secret, "1h");
...
const getMe = async (session: any) => {
    ...
    const decodedData = jwtHelpers.verifyToken(accessToken, config.jwt.access_token_secret as Secret);
